...
class ReportController extends Controller
{
    ...
    public function monthlyfilter($id, $monthStart, $monthEnd, $year) {
        ...
        if ($user['role_id'] == 2) {
            $query = "
                ...
                FROM (
                    ...
                    WHERE MONTH(a.booking_date) BETWEEN $mstart AND $mend AND YEAR(a.booking_date) = $year AND c.corporate_manager_id = $id
            ...
            $query = "
                ...
                FROM (
                    ...
                    INNER JOIN services b ON a.services_id = b.id
                    INNER JOIN service_centers c ON a.service_center_id = c.id
                    WHERE MONTH(a.booking_date) BETWEEN $mstart AND $mend AND YEAR(a.booking_date) = $year AND c.id = $sc_id
                    GROUP BY c.name, MONTHNAME(a.booking_date), b.name
                    ORDER BY c.name, b.name, MONTHNAME(a.booking_date) ASC
                ) AS subquery
                GROUP BY subquery.service_center, subquery.services
    ...
    public function monthDay($id, $month, $year) {
        $user = User::where('id' ,'=', $id)->first();
        ...
        if ($user['role_id'] == 2) {
            ...
            $query = "
                ...
                FROM (
                    ...
                    INNER JOIN services b ON a.services_id = b.id
                    INNER JOIN service_centers c ON a.service_center_id = c.id
                    WHERE MONTH(a.booking_date) = $month AND YEAR(a.booking_date) = $year and c.corporate_manager_id = $id
            ...
            $query = "
                ...
                FROM (
                    ...
                    INNER JOIN services b ON a.services_id = b.id
                    INNER JOIN service_centers c ON a.service_center_id = c.id
                    WHERE MONTH(a.booking_date) = $month AND YEAR(a.booking_date) = $year and c.id = $sc_id
                    GROUP BY c.name, DAY(a.booking_date), b.name
                ) b
                GROUP BY b.service_center, b.services
                ORDER BY b.service_center, b.services
        ...
        if ($user['role_id'] == 2) {
            $yearly = DB::select("
                ...
                FROM
                    ...
                    WHERE b.corporate_manager_id = $id
                GROUP BY
                    services,
                    service_center
                ORDER BY
    ...
    public function yearlyfilter($id, $yearStart, $yearEnd) {
        $user = User::where('id' ,'=', $id)->first();
        $cols = '';
        $select_cols = '';
        ...
        if ($user['role_id'] == 2) {
            $query = "
                ...
                FROM (
                    ...
                    INNER JOIN services b ON a.services_id = b.id
                    INNER JOIN service_centers c ON a.service_center_id = c.id
                    WHERE year(a.booking_date) between $yearStart AND $yearEnd AND c.corporate_manager_id = $id
            ...
            $query = "
                ...
                FROM (
                    ...
                    INNER JOIN services b ON a.services_id = b.id
                    INNER JOIN service_centers c ON a.service_center_id = c.id
                    WHERE year(a.booking_date) between $yearStart AND $yearEnd AND c.id = $sc_id
                    GROUP BY c.name, year(a.booking_date), b.name
                    ORDER BY c.name, b.name, year(a.booking_date)
                ) AS subquery
                GROUP BY subquery.service_center, subquery.services
    ...
    public function yearMonth($id, $year) {
        $user = User::where('id' ,'=', $id)->first();
        ...
        if ($user['role_id'] == 2) {
            ...
            FROM
                ...
                INNER JOIN service_centers b ON a.service_center_id = b.id
                INNER JOIN services c ON a.services_id = c.id
                WHERE YEAR(a.booking_date) = $year AND b.corporate_manager_id = $id
            GROUP BY
                services,
                service_center
            ORDER BY
            ...
            FROM
                ...
                INNER JOIN service_centers b ON a.service_center_id = b.id
                INNER JOIN services c ON a.services_id = c.id
                WHERE YEAR(a.booking_date) = $year AND b.id = $sc_id
            GROUP BY
                services,
                service_center
            ORDER BY
